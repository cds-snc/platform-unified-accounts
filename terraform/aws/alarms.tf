...
module "alarms_slack" {
  ...
  name      = "alarms-slack"
  ecr_arn   = aws_ecr_repository.repo["alarms-slack"].arn
  image_uri = "${aws_ecr_repository.repo["alarms-slack"].repository_url}:latest"
...
resource "aws_cloudwatch_metric_alarm" "idp_cleanup_users_dlq_messages" {
  ...
  dimensions = {
    QueueName = aws_sqs_queue.idp_event_cleanup_users_dlq_queue.name
  }
